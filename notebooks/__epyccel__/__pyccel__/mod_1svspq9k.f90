...
function solve_2d_nonlinearconv_pyccel(u, un, v, vn, nt, nx, ny, dt, dx, &
  ...
  real(C_DOUBLE), intent(inout), target :: un(0:,0:)
  real(C_DOUBLE), intent(inout), target :: v(0:,0:)
  real(C_DOUBLE), intent(inout), target :: vn(0:,0:)
  ...
  do n = 0_C_LONG_LONG, nt-1_C_LONG_LONG, 1_C_LONG_LONG
    ...
    un = u
    if (allocated(u)) then
      if (any(size(u) /= [size(vn, 1), size(vn, 2)])) then
        deallocate(u)
        allocate(u(0:size(vn, 1) - 1_C_LONG_LONG, 0:size(vn, 2) - &
      1_C_LONG_LONG))
      end if
    else
      allocate(u(0:size(vn, 1) - 1_C_LONG_LONG, 0:size(vn, 2) - &
    ...
    vn = v
    if (allocated(v)) then
      if (any(size(v) /= [size(un, 1), size(un, 2)])) then
        deallocate(v)
        allocate(v(0:size(un, 1) - 1_C_LONG_LONG, 0:size(un, 2) - &
      1_C_LONG_LONG))
      end if
    else
      allocate(v(0:size(un, 1) - 1_C_LONG_LONG, 0:size(un, 2) - &
